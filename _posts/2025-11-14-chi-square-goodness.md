...
where $n$ is the number of data points, $y_i$ is the observed value, $\hat{y}_i$ is the expected value, and $\sigma_i$ is the standard deviation (error) of the observed value. The $\chi^2$ sum is a square sum of the residuals, which are normalised. The smaller the $\chi^2$ sum, the better the fit. So you can try different $a$s and $b$s to find the minimum $\chi^2$ sum. The values of $a$ and $b$ that give the minimum $\chi^2$ sum are the best-fit parameters.
